...
function markdownSemana(iso) {
  ...
  const max = fcMaxima();
  const zonaDe = fc => !max ? '' : ` (Z${fc / max < .6 ? 1 : fc / max < .7 ? 2 : fc / max < .8 ? 3 : fc / max < .9 ? 4 : 5})`;
  const L = [];
  ...
  if (max) L.push(`- Mis zonas: Z1 <${Math.round(max * .6)} · Z2 ${Math.round(max * .6)}-${Math.round(max * .7)} · Z3 ${Math.round(max * .7)}-${Math.round(max * .8)} · Z4 ${Math.round(max * .8)}-${Math.round(max * .9)} · Z5 >${Math.round(max * .9)} ppm`);
  L.push('');
  ...
  L.push('---', '', `_Generado por mi diario de entrenos el ${fechaLarga(hoyISO())}. Los 1RM son estimaciones (fórmula de Epley) y las zonas de FC salen de la FC media de cada sesión, no del registro segundo a segundo._`);
