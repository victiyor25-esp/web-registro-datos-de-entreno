...
const VERSION_CODIGO = 'v4';
console.info('Diario de Entrenos —', VERSION_CODIGO);
...
const PERFIL_DEFECTO = {
  ...
  metas: [], prsManual: {}, descansos: []
};

let usuario = null;
let datos = { running: [], bici: [], fuerza: [], semanas: [], perfil: { ...PERFIL_DEFECTO } };
...
/* Mensaje discreto que se va solo, en vez de un cuadro del navegador */
...
/* Duración escrita a mano: acepta 46:15, 1:12:30, 46 y 46,25.
   Devuelve minutos decimales, o null si no hay forma de entenderlo. */
function duracionDeTexto(t) {
  const s = String(t ?? '').trim().replace(',', '.');
  if (!s) return null;
  if (!/^\d+(\.\d+)?(:\d{1,2}(\.\d+)?){0,2}$/.test(s)) return null;
  const p = s.split(':').map(Number);
  if (p.some(isNaN)) return null;
  const min = p.length === 3 ? p[0] * 60 + p[1] + p[2] / 60
    : p.length === 2 ? p[0] + p[1] / 60
      : p[0];
  return min > 0 ? Math.round(min * 1000) / 1000 : null;
}
...
async function cargarTodo() {
  const [run, bic, fue, sem, perfilSnap] = await Promise.all([
    col('running').get(), col('bici').get(), col('fuerza').get(), col('semanas').get(), refUsuario().get()
  ]);
  const mapear = s => s.docs.map(d => ({ id: d.id, ...d.data() }));
  datos.running = mapear(run).sort((a, b) => a.fecha.localeCompare(b.fecha));
  datos.bici = mapear(bic).sort((a, b) => a.fecha.localeCompare(b.fecha));
...
function pintarUsuario() {
  ...
  $$('.marca-version').forEach(el => { el.textContent = VERSION_CODIGO; });
  const f = $('#form-perfil');
...
/* 4) NAVEGACIÓN                                                       */
/* ------------------------------------------------------------------ */
const ORDEN_TABS = ['resumen', 'records', 'running', 'bici', 'fuerza', 'semana', 'historial', 'cuenta'];
let tabActual = 'resumen';

let salidaEnCurso = null;

function irA(tab) {
  if (tab === tabActual) return;
  const desde = ORDEN_TABS.indexOf(tabActual), hasta = ORDEN_TABS.indexOf(tab);
  const haciaDelante = hasta >= desde;
  const anterior = $('#' + tabActual);
  tabActual = tab;

  // limpia una salida anterior sin terminar
  if (salidaEnCurso) {
    clearTimeout(salidaEnCurso.reloj);
    salidaEnCurso.el.classList.remove('saliendo', 'sale-izq', 'sale-der', 'active');
    salidaEnCurso = null;
  }
  ...
  $$('.tab-panel').forEach(p => {
    if (p === anterior) return;
    const activo = p.id === tab;
    p.classList.remove('entra-der', 'entra-izq', 'saliendo', 'sale-izq', 'sale-der');
    p.classList.toggle('active', activo);
    if (activo) {
      void p.offsetWidth;
      p.classList.add(haciaDelante ? 'entra-der' : 'entra-izq');
    }
  });

  // el panel que dejamos se aparta hacia el lado contrario
  if (anterior && anterior.id !== tab) {
    anterior.classList.add('saliendo', haciaDelante ? 'sale-izq' : 'sale-der');
    const reloj = setTimeout(() => {
      anterior.classList.remove('saliendo', 'sale-izq', 'sale-der', 'active');
      salidaEnCurso = null;
    }, 380);
    salidaEnCurso = { el: anterior, reloj };
  }

  // el fondo se reencuadra y vuelve a entrar en cada pestaña
  const fondo = $('#fondo');
  if (fondo && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
    fondo.style.setProperty('--fx', (18 + hasta * 9) + '%');
    fondo.classList.remove('fondo-entra');
    void fondo.offsetWidth;
    fondo.classList.add('fondo-entra');
  }

  window.scrollTo({ top: 0, behavior: 'smooth' });
  if (tab === 'resumen') setTimeout(animarContadores, 520);
}

/* La navegación se separa del fondo al desplazar */
window.addEventListener('scroll', () => {
  $('#tabs').classList.toggle('fijada', window.scrollY > 12);
}, { passive: true });

/* Los números suben hasta su valor en vez de aparecer de golpe */
function animarContadores() {
  if (matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  $$('[data-cuenta]').forEach(el => {
    const fin = parseFloat(el.dataset.cuenta);
    ...
    const paso = t => {
      ...
      el.textContent = num(fin * suave, dec);
      if (p < 1) requestAnimationFrame(paso);
      else el.textContent = num(fin, dec);
    };
...
function semanaActual() {
  ...
  const bicis = datos.bici.filter(e => claveSemana(e.fecha) === inicio);
  ...
  return {
    ...
    kmBici: bicis.reduce((s, e) => s + e.distancia, 0),
    sesiones: runs.length + bicis.length + sesionesFuerza,
    sesionesRunning: runs.length,
    sesionesBici: bicis.length,
    sesionesFuerza,
    fechas: new Set([...runs.map(e => e.fecha), ...bicis.map(e => e.fecha), ...fue.map(e => e.fecha)])
  };
}

function agregadosSemana(iso) {
  const runs = datos.running.filter(e => claveSemana(e.fecha) === iso);
  const bicis = datos.bici.filter(e => claveSemana(e.fecha) === iso);
  const fue = datos.fuerza.filter(e => claveSemana(e.fecha) === iso);
  const diasFuerza = new Set(fue.map(e => e.fecha)).size;
  const rpes = [...runs, ...bicis].map(e => e.rpe).filter(v => v != null && !isNaN(v));
  const conDolor = runs
    .map(e => ({ v: Math.max(e.rodilla || 0, e.dolorValor || 0), zona: e.rodilla >= (e.dolorValor || 0) && e.rodilla > 0 ? 'rodilla' : (e.dolorZona || '') }))
    .filter(x => x.v > 0)
    .sort((a, b) => b.v - a.v);
  return {
    iso,
    km: runs.reduce((s, e) => s + e.distancia, 0),
    kmBici: bicis.reduce((s, e) => s + e.distancia, 0),
    nRunning: runs.length,
    nBici: bicis.length,
    diasFuerza,
    volumen: fue.reduce((s, e) => s + (e.series || 0) * (e.reps || 0) * (e.peso || 0), 0),
    sesiones: runs.length + bicis.length + diasFuerza,
    rpeMedia: rpes.length ? Math.round(rpes.reduce((a, b) => a + b, 0) / rpes.length) : null,
    dolor: conDolor.length ? conDolor[0].v : 0,
    zonaDolor: conDolor.length ? conDolor[0].zona : ''
  };
}

/* Rellena el check-in con lo que ya sabemos de esa semana.
   `forzar` sobrescribe también lo que hubieras escrito a mano. */
function rellenarSemana(iso, forzar) {
  const f = $('#form-semana');
  const a = agregadosSemana(iso);
  const guardado = datos.semanas.find(s => s.fecha === iso);

  f.fecha.value = iso;
  f.kmTotales.value = a.km ? a.km.toFixed(2) : '';
  f.entrenosCompletados.value = a.sesiones || '';
  f.entrenosPlaneados.value = datos.perfil.objetivoSesiones || '';
  if (forzar || !f.fatiga.value || f.fatiga.value === '4') f.fatiga.value = a.rpeMedia ?? 4;
  if (forzar || !f.dolorValor.value || f.dolorValor.value === '0') f.dolorValor.value = a.dolor;
  if (forzar || !f.dolorZona.value) f.dolorZona.value = a.zonaDolor;

  // lo que solo puedes saber tú: si ya lo guardaste esa semana, lo recuperamos
  if (guardado) {
    if (forzar || !f.peso.value) f.peso.value = guardado.peso ?? '';
    if (forzar || !f.suenoHoras.value) f.suenoHoras.value = guardado.suenoHoras ?? '';
    if (forzar) f.suenoCalidad.value = guardado.suenoCalidad ?? 3;
    if (forzar || !f.destacable.value) f.destacable.value = guardado.destacable || '';
  } else if (forzar) {
    f.peso.value = ''; f.suenoHoras.value = ''; f.destacable.value = '';
  }

  const partes = [];
  if (a.nRunning) partes.push(`${a.nRunning} ${a.nRunning === 1 ? 'carrera' : 'carreras'} (${num(a.km)} km)`);
  if (a.nBici) partes.push(`${a.nBici} ${a.nBici === 1 ? 'salida' : 'salidas'} en bici (${num(a.kmBici)} km)`);
  if (a.diasFuerza) partes.push(`${a.diasFuerza} ${a.diasFuerza === 1 ? 'sesión' : 'sesiones'} de fuerza (${num(a.volumen, 0)} kg)`);
  const faltan = [];
  if (!f.peso.value) faltan.push('el peso');
  if (!f.suenoHoras.value) faltan.push('el sueño');
  const cola = faltan.length ? ` Solo te queda ${faltan.join(' y ')}.` : ' No falta nada por escribir.';
  $('#resumen-semana').innerHTML = partes.length
    ? `Encontrado en esa semana: <strong style="color:var(--accent-2)">${partes.join(' · ')}</strong>.${cola}`
    : 'No hay entrenos guardados en esa semana.';

  $('#btn-guardar-semana').textContent = guardado ? 'Actualizar check-in' : 'Guardar check-in';
  pintarDescansos(iso);
  renderPlan(iso);
}

function pintarDescansos(iso) {
  const cont = $('#semana-descansos');
  if (!cont) return;
  if (!iso) { cont.innerHTML = ''; return; }
  const nombres = ['L', 'M', 'X', 'J', 'V', 'S', 'D'];
  const lunes = desdeISO(iso);
  const set = descansosSet();
  cont.innerHTML = nombres.map((n, i) => {
    const d = new Date(lunes); d.setDate(lunes.getDate() + i);
    const dia = isoLocal(d);
    const entrenado = datos.running.some(e => e.fecha === dia)
      || datos.bici.some(e => e.fecha === dia)
      || datos.fuerza.some(e => e.fecha === dia);
    return `<button type="button" class="dia-descanso ${set.has(dia) ? 'on' : ''}${entrenado ? ' entrenado' : ''}"
      data-descanso="${dia}" title="${fechaLarga(dia)}${entrenado ? ' — ese día hay entrenos guardados' : ''}">${n}</button>`;
  }).join('');
}
...
/* Caminar y el descanso activo son actividad, pero su ritmo no es de carrera:
   se quedan fuera de los récords para no ensuciarlos. */
const TIPOS_SIN_RITMO = ['Caminar', 'Descanso activo'];
const esCarreraReal = e => !TIPOS_SIN_RITMO.includes(e.tipo);

/* Días marcados como descanso completo */
const descansosSet = () => new Set(datos.perfil.descansos || []);
function descansosDe(iso) {
  const lunes = desdeISO(iso);
  const set = descansosSet();
  const out = [];
  for (let i = 0; i < 7; i++) {
    const d = new Date(lunes); d.setDate(lunes.getDate() + i);
    const dia = isoLocal(d);
    if (set.has(dia)) out.push(dia);
  }
  return out;
}

/* ---- Plan semanal pegado por el usuario -------------------------- */
const sinTildes = s => String(s).normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
const DIAS_CLAVE = ['lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado', 'domingo'];
const planDe = iso => (datos.perfil.planes || {})[iso] || '';

/* Reparte el texto del plan en los siete días. Tolera viñetas, negritas
   y numeraciones; lo que va suelto se pega al último día nombrado. */
function parsearPlan(texto) {
  const out = ['', '', '', '', '', '', ''];
  let actual = -1;
  String(texto || '').split('\n').forEach(linea => {
    const limpia = linea.replace(/\*\*/g, '').replace(/^[\s\-*#>•·\d.)]+/, '').trim();
    if (!limpia) return;
    const plano = sinTildes(limpia);
    const i = DIAS_CLAVE.findIndex(d => plano.startsWith(d));
    if (i >= 0) {
      actual = i;
      const resto = limpia.slice(DIAS_CLAVE[i].length).replace(/^[\s:.\-–—]+/, '').trim();
      out[i] = resto;
    } else if (actual >= 0) {
      out[actual] += (out[actual] ? ' · ' : '') + limpia;
    }
  });
  return out;
}

/* Qué hice realmente un día, en una línea */
function resumenDia(dia) {
  const partes = [];
  datos.running.filter(e => e.fecha === dia)
    .forEach(e => partes.push(`${num(e.distancia, 2)} km · ${e.tipo.toLowerCase()}`));
  datos.bici.filter(e => e.fecha === dia)
    .forEach(e => partes.push(`${num(e.distancia, 2)} km en bici`));
  const fue = datos.fuerza.filter(e => e.fecha === dia);
  if (fue.length) partes.push(`fuerza: ${fue.map(x => x.ejercicio).join(', ')}`);
  if (!partes.length && descansosSet().has(dia)) return 'descanso completo';
  return partes.join(' · ');
}

/* Carga aguda (7 días) frente a la crónica (las 4 semanas anteriores).
   Con ventanas móviles, no por semanas naturales: así la semana en curso
   no aparece siempre baja por estar a medias. */
function cargaRelativa() {
  const hoy = new Date(); hoy.setHours(0, 0, 0, 0);
  const kmEntre = (desde, hasta) => datos.running
    .filter(e => { const d = desdeISO(e.fecha); return d >= desde && d <= hasta; })
    .reduce((s, e) => s + (e.distancia || 0), 0);
  const ini7 = new Date(hoy); ini7.setDate(ini7.getDate() - 6);
  const fin28 = new Date(hoy); fin28.setDate(fin28.getDate() - 7);
  const ini28 = new Date(hoy); ini28.setDate(ini28.getDate() - 34);
  const aguda = kmEntre(ini7, hoy);
  const cronica = kmEntre(ini28, fin28) / 4;
  const molestias = datos.running.filter(e => {
    const d = desdeISO(e.fecha);
    return d >= ini7 && d <= hoy && ((e.rodilla || 0) >= 4 || (e.dolorValor || 0) >= 4);
  }).length;
  const ratio = cronica > 0 ? aguda / cronica : null;
  let nivel = 'sin-datos', texto = '', detalle = '';
  if (ratio === null) {
    texto = 'Aún no hay cinco semanas de carrera para comparar la carga.';
  } else if (ratio > 1.5) {
    nivel = 'alto';
    texto = 'Has subido la carga muy rápido';
    detalle = `${num(aguda, 1)} km en los últimos 7 días frente a una media de ${num(cronica, 1)} km. Un salto así es cuando más lesiones aparecen: baja el volumen esta semana.`;
  } else if (ratio > 1.3) {
    nivel = 'medio';
    texto = 'Subida de carga por encima de lo prudente';
    detalle = `${num(aguda, 1)} km en 7 días frente a ${num(cronica, 1)} km de media. Mantén aquí sin subir más y vigila la rodilla.`;
  } else if (ratio < 0.7) {
    nivel = 'bajo';
    texto = 'Carga bastante por debajo de tu media';
    detalle = `${num(aguda, 1)} km en 7 días frente a ${num(cronica, 1)} km de media. Bien si es semana de descarga; si no, te estás desentrenando.`;
  } else {
    nivel = 'ok';
    texto = 'Carga estable';
    detalle = `${num(aguda, 1)} km en los últimos 7 días, en línea con tu media de ${num(cronica, 1)} km.`;
  }
  if (molestias >= 2) {
    nivel = nivel === 'alto' ? 'alto' : 'medio';
    texto = 'Molestias repetidas esta semana';
    detalle = `${molestias} entrenos con dolor de 4 o más sobre 10 en los últimos 7 días. ` + detalle;
  }
  return { aguda, cronica, ratio, molestias, nivel, texto, detalle };
}
...
/* Zonas de frecuencia cardiaca fijadas en pulsaciones reales, no como
   porcentaje de la FC máxima: son las que me ha dado mi test. */
const ZONAS_FC = [
  { nombre: 'Z1 Recuperación', desde: 108, hasta: 127, color: '#5c7cff' },
  { nombre: 'Z2 Aeróbico', desde: 127, hasta: 145, color: '#b6ff2e' },
  { nombre: 'Z3 Tempo', desde: 145, hasta: 163, color: '#ffcd50' },
  { nombre: 'Z4 Umbral', desde: 163, hasta: 172, color: '#ff7a1a' },
  { nombre: 'Z5 VO2máx', desde: 172, hasta: 181, color: '#ff5c5c' }
];

/* Devuelve el número de zona (1-5) de una FC, o null si queda fuera de rango.
   El límite inferior entra en la zona; el superior pasa a la siguiente. */
function numeroZona(fc) {
  if (!fc) return null;
  if (fc < ZONAS_FC[0].desde) return null;
  if (fc >= ZONAS_FC[4].hasta) return 5;
  const i = ZONAS_FC.findIndex(z => fc >= z.desde && fc < z.hasta);
  return i >= 0 ? i + 1 : null;
}

const rangoZona = z => `${z.desde} - ${z.hasta} ppm`;
...
function zonasFC() {
  ...
  const lista = ZONAS_FC.map(z => ({ ...z, minutos: 0 }));
  conFC.forEach(e => {
    const n = numeroZona(e.fcMedia);
    if (n) lista[n - 1].minutos += e.duracion;
  });
  const total = lista.reduce((s, z) => s + z.minutos, 0);
  return {
    max: fcMaxima(), total, entrenos: conFC.length,
    lista: lista.map(z => ({
      ...z,
...
function markdownSemana(iso) {
  const a = agregadosSemana(iso);
  const chk = datos.semanas.find(s => s.fecha === iso);
  const lunes = desdeISO(iso);
  const domingo = new Date(lunes); domingo.setDate(domingo.getDate() + 6);
  const siguiente = new Date(lunes); siguiente.setDate(siguiente.getDate() + 7);
  const max = fcMaxima();
  const zonaDe = fc => { const n = numeroZona(fc); return n ? ` (Z${n})` : ''; };
  const L = [];

  L.push(`# Entrenamiento — semana del ${fechaLarga(iso)} al ${fechaLarga(isoLocal(domingo))}`, '');
  L.push('## Lo que necesito de ti', '');
  L.push(`Eres mi entrenador. Con los datos de esta semana y el contexto de abajo, prepárame el plan de la semana del ${fechaLarga(isoLocal(siguiente))}: qué sesión toca cada día, con distancia o duración, intensidad (ritmo o zona de FC) y qué hacer de fuerza. Ajusta la carga según cómo he acabado esta semana y avísame si ves riesgo de lesión o de exceso.`, '');

  L.push('## Mi perfil', '');
  if (datos.perfil.edad) L.push(`- Edad: ${datos.perfil.edad} años`);
  L.push(`- FC máxima: ${max ? `${max} ppm${datos.perfil.fcMax ? '' : ' (estimada)'}` : 'sin dato'}`);
  if (datos.perfil.objetivoKm) L.push(`- Objetivo semanal: ${datos.perfil.objetivoKm} km`);
  if (datos.perfil.objetivoSesiones) L.push(`- Sesiones por semana previstas: ${datos.perfil.objetivoSesiones}`);
  L.push(`- Mis zonas de FC (medidas, no estimadas): ${ZONAS_FC.map((z, i) => `Z${i + 1} ${z.desde}-${z.hasta}`).join(' · ')} ppm`);
  L.push('');

  L.push('## Cómo ha ido esta semana', '');
  if (a.km) L.push(`- Carrera: ${num(a.km, 2)} km en ${a.nRunning} ${a.nRunning === 1 ? 'sesión' : 'sesiones'}${a.km && a.nRunning ? ` · ritmo medio ${fmtRitmo(ritmo(a.km, datos.running.filter(e => claveSemana(e.fecha) === iso).reduce((s, e) => s + e.duracion, 0)))} /km` : ''}`);
  if (a.kmBici) L.push(`- Bici: ${num(a.kmBici, 2)} km en ${a.nBici} ${a.nBici === 1 ? 'salida' : 'salidas'}`);
  if (a.diasFuerza) L.push(`- Fuerza: ${a.diasFuerza} ${a.diasFuerza === 1 ? 'sesión' : 'sesiones'} · ${num(a.volumen, 0)} kg movidos`);
  L.push(`- Sesiones totales: ${a.sesiones}${chk?.entrenosPlaneados ? ` de ${chk.entrenosPlaneados} planeadas` : ''}`);
  const nDesc = descansosDe(iso).length;
  L.push(`- Días de descanso completo marcados: ${nDesc}`);
  const carga = cargaRelativa();
  if (carga.ratio !== null) {
    L.push(`- Carga de los últimos 7 días: ${num(carga.aguda, 1)} km frente a una media de ${num(carga.cronica, 1)} km en las 4 semanas anteriores (ratio ${num(carga.ratio, 2)}). ${carga.texto}.`);
  }
  if (datos.perfil.objetivoKm) L.push(`- Cumplimiento del objetivo: ${Math.round((a.km / datos.perfil.objetivoKm) * 100)}%`);
  if (a.rpeMedia != null) L.push(`- RPE medio: ${a.rpeMedia}/10`);
  if (chk) {
    if (chk.peso) L.push(`- Peso: ${num(chk.peso, 2)} kg`);
    if (chk.suenoHoras) L.push(`- Sueño: ${num(chk.suenoHoras, 2)} h de media, calidad ${chk.suenoCalidad}/5`);
    if (chk.fatiga != null) L.push(`- Fatiga al cerrar la semana: ${chk.fatiga}/10`);
    if (chk.dolorValor) L.push(`- Dolor: ${chk.dolorValor}/10${chk.dolorZona ? ` en ${chk.dolorZona}` : ''}`);
    else L.push('- Dolor: ninguno');
  } else {
    L.push('- Check-in semanal: sin rellenar (no tengo peso, sueño ni fatiga de esta semana)');
  }
  L.push('');

  L.push('## Check-in semanal, tal como lo guardé', '');
  if (!chk) {
    L.push('- No rellené el check-in de esta semana.');
  } else {
    L.push(`- Semana del: ${fechaLarga(chk.fecha)}`);
    L.push(`- Peso: ${chk.peso != null ? `${num(chk.peso, 2)} kg` : 'sin apuntar'}`);
    L.push(`- Km de carrera anotados: ${chk.kmTotales != null ? `${num(chk.kmTotales, 2)} km` : 'sin apuntar'}`);
    L.push(`- Sesiones completadas: ${chk.entrenosCompletados ?? 'sin apuntar'}`);
    L.push(`- Sesiones planeadas: ${chk.entrenosPlaneados ?? 'sin apuntar'}`);
    L.push(`- Sueño: ${chk.suenoHoras != null ? `${num(chk.suenoHoras, 2)} h de media` : 'sin apuntar'}`);
    L.push(`- Calidad del sueño: ${chk.suenoCalidad ?? 'sin apuntar'}/5`);
    L.push(`- Fatiga: ${chk.fatiga ?? 'sin apuntar'}/10`);
    L.push(`- Dolor: ${chk.dolorValor ?? 0}/10${chk.dolorZona ? ` en ${chk.dolorZona}` : ''}`);
  }
  L.push('');

  if (chk?.destacable) L.push('## Mis sensaciones y observaciones de la semana', '', chk.destacable, '');

  const prs = calcularRecords().filter(r => r.tiempo);
  if (prs.length) {
    L.push('## Mis récords actuales', '');
    prs.forEach(r => L.push(`- ${r.label}: ${fmtTiempo(r.tiempo)} (${fmtRitmo(r.tiempo / r.d)} /km, ${fechaLarga(r.fecha)})${r.exacto ? '' : ' — estimado a partir de un entreno más largo'}`));
    L.push('');
  }

  const metas = datos.perfil.metas || [];
  if (metas.length) {
    L.push('## Mis objetivos', '');
    metas.forEach(m => {
      const p = progresoMeta(m, calcularRecords());
      L.push(`- ${m.nombre} — ${p.estado}. ${p.detalle}`);
    });
    L.push('');
  }

  // estado de la rodilla, día a día de esta semana
  const runsSemana = datos.running.filter(e => claveSemana(e.fecha) === iso).sort((a, b) => a.fecha.localeCompare(b.fecha));
  L.push('## Rodilla y molestias de esta semana', '');
  if (!runsSemana.length) {
    L.push('- Sin carreras esta semana, así que no hay registro de rodilla.');
  } else {
    runsSemana.forEach(e => {
      const trozos = [`rodilla ${e.rodilla || 0}/10`];
      if (e.dolorValor) trozos.push(`${e.dolorZona || 'otro dolor'} ${e.dolorValor}/10`);
      L.push(`- ${fechaLarga(e.fecha)} (tras ${num(e.distancia, 2)} km de ${e.tipo}): ${trozos.join(' · ')}`);
    });
    const peor = Math.max(...runsSemana.map(e => e.rodilla || 0));
    L.push(`- Peor valor de rodilla de la semana: ${peor}/10${peor === 0 ? ' (ninguna molestia)' : ''}.`);
  }
  if (chk?.dolorValor) L.push(`- En el check-in del domingo apunté ${chk.dolorValor}/10${chk.dolorZona ? ` en ${chk.dolorZona}` : ''}.`);
  L.push('');

  const planSemana = parsearPlan(planDe(iso));
  if (planSemana.some(Boolean)) {
    L.push('## El plan que tenía y lo que hice', '');
    planSemana.forEach((texto, i) => {
      const d = new Date(lunes); d.setDate(lunes.getDate() + i);
      const dia = isoLocal(d);
      const hecho = resumenDia(dia);
      L.push(`- **${DIAS_LARGOS[i]}** — previsto: ${texto || 'nada'} · hecho: ${hecho || 'nada registrado'}`);
    });
    L.push('');
  }

  const descansosMarcados = new Set(descansosDe(iso));
  L.push('## Entrenos día a día', '');
  let algo = false;
  for (let i = 0; i < 7; i++) {
    const d = new Date(lunes); d.setDate(lunes.getDate() + i);
    const dia = isoLocal(d);
    const runs = datos.running.filter(e => e.fecha === dia);
    const bicis = datos.bici.filter(e => e.fecha === dia);
    const fue = datos.fuerza.filter(e => e.fecha === dia);
    if (!runs.length && !bicis.length && !fue.length) {
      L.push(`### ${DIAS_LARGOS[i]} ${fechaCorta(dia)}`, '',
        descansosMarcados.has(dia)
          ? '- Descanso completo (marcado por mí a propósito)'
          : '- Sin nada registrado (no marqué descanso, puede que olvidara apuntarlo)', '');
      continue;
    }
    algo = true;
    L.push(`### ${DIAS_LARGOS[i]} ${fechaCorta(dia)}`, '');
    runs.forEach(e => {
      L.push(`- **Carrera — ${e.tipo}**`);
      L.push(`  - Distancia: ${num(e.distancia, 2)} km`);
      L.push(`  - Duración: ${num(e.duracion, 2)} min (${fmtTiempo(e.duracion)})`);
      L.push(`  - Ritmo: ${fmtRitmo(ritmo(e.distancia, e.duracion))} /km`);
      L.push(`  - FC media: ${e.fcMedia ? `${e.fcMedia} ppm${zonaDe(e.fcMedia)}` : 'sin apuntar'}`);
      L.push(`  - FC máxima: ${e.fcMaxima ? `${e.fcMaxima} ppm` : 'sin apuntar'}`);
      L.push(`  - RPE: ${e.rpe != null ? `${e.rpe}/10` : 'sin apuntar'}`);
      L.push(`  - Rodilla: ${e.rodilla || 0}/10${e.rodilla ? '' : ' (sin molestias)'}`);
      L.push(`  - Otro dolor: ${e.dolorValor ? `${e.dolorValor}/10${e.dolorZona ? ` en ${e.dolorZona}` : ''}` : 'ninguno'}`);
      if (e.notas) L.push(`  - Sensaciones: ${e.notas}`);
    });
    bicis.forEach(e => {
      L.push(`- **Bici — ${e.tipo}**`);
      L.push(`  - Distancia: ${num(e.distancia, 2)} km`);
      L.push(`  - Duración: ${num(e.duracion, 2)} min (${fmtTiempo(e.duracion)})`);
      if (e.duracion) L.push(`  - Velocidad media: ${num(e.distancia / (e.duracion / 60), 1)} km/h`);
      L.push(`  - Desnivel: ${e.desnivel ? `${num(e.desnivel, 0)} m D+` : 'sin apuntar'}`);
      L.push(`  - FC media: ${e.fcMedia ? `${e.fcMedia} ppm${zonaDe(e.fcMedia)}` : 'sin apuntar'}`);
      L.push(`  - RPE: ${e.rpe != null ? `${e.rpe}/10` : 'sin apuntar'}`);
      if (e.notas) L.push(`  - Sensaciones: ${e.notas}`);
    });
    if (fue.length) {
      const vol = fue.reduce((s, e) => s + (e.series || 0) * (e.reps || 0) * (e.peso || 0), 0);
      L.push(`- **Fuerza** — ${num(vol, 0)} kg en total:`);
      fue.forEach(e => L.push(`  - ${e.ejercicio}: ${e.series} series × ${e.reps} reps @ ${num(e.peso, 2)} kg · volumen ${num(e.series * e.reps * e.peso, 0)} kg · 1RM estimado ${num(oneRM(e.peso, e.reps), 1)} kg${e.notas ? ` — ${e.notas}` : ''}`));
    }
    L.push('');
  }
  if (!algo) L.push('_No hay ningún entreno registrado en esta semana._', '');

  L.push('---', '', `_Generado por mi diario de entrenos el ${fechaLarga(hoyISO())}. Los 1RM son estimaciones (fórmula de Epley) y la zona de FC de cada sesión sale de su FC media, no del registro segundo a segundo._`);

  return L.join('\n');
}

function descargarMD(iso) {
  const blob = new Blob([markdownSemana(iso)], { type: 'text/markdown;charset=utf-8' });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `semana-${iso}.md`;
  a.click();
...
function graficaCombo() {
  ...
  // eje derecho: ritmo min/km (más arriba = más rápido). Con una sola semana
  // no hay escala que representar, así que se omite.
  if (rs.length >= 2) {
...
function renderResumen() {
  ...
  $('#racha-semanas').dataset.cuenta = racha;
  $('#racha-semanas').dataset.decimales = 0;
  ...
  const descansos = descansosSet();
  $('#semana-dias').innerHTML = nombres.map((n, i) => {
    const d = new Date(lunes); d.setDate(lunes.getDate() + i);
    const iso = isoLocal(d);
    const descanso = !sem.fechas.has(iso) && descansos.has(iso);
    return `<div class="dia ${sem.fechas.has(iso) ? 'on' : ''} ${descanso ? 'descanso' : ''} ${i === indiceHoy ? 'hoy' : ''}" title="${fechaLarga(iso)}${descanso ? ' — descanso completo' : ''}">
  ...
  $('#objetivos-lista').innerHTML = [
    ...
    { nombre: 'Bici', valor: sem.kmBici ? `${num(sem.kmBici)} km` : '—', pct: Math.min(100, sem.sesionesBici * 33), color: '#5c7cff' },
    ...
    <div class="objetivo-fila">
      ...
      <div class="pista"><div style="width:0%;background:${o.color}" data-ancho="${o.pct}%"></div></div>
    </div>`).join('');

  requestAnimationFrame(() => $$('#objetivos-lista .pista > div').forEach(d => { d.style.width = d.dataset.ancho; }));
  ...
  $('#cards-resumen').innerHTML = [
    ...
    { valor: num(totalKm), cuenta: totalKm, dec: 1, etiqueta: 'Km acumulados', sub: `${semanas.length} ${semanas.length === 1 ? 'semana' : 'semanas'} registradas`, color: 'var(--text)' },
    { valor: num(datos.bici.reduce((s, e) => s + e.distancia, 0)), cuenta: datos.bici.reduce((s, e) => s + e.distancia, 0), dec: 1, etiqueta: 'Km en bici', sub: `${datos.bici.length} ${datos.bici.length === 1 ? 'salida' : 'salidas'}`, color: '#5c7cff' },
    { valor: nSesFuerza, cuenta: nSesFuerza, dec: 0, etiqueta: 'Sesiones de fuerza', sub: `${num(totalVol / 1000, 1)} t movidas`, color: 'var(--accent-2)' },
    {
    ...
    <div class="card">
      <div class="valor" style="color:${c.color}"${c.cuenta != null ? ` data-cuenta="${c.cuenta}" data-decimales="${c.dec ?? 1}"` : ''}>${c.valor}</div>
  ...
  // zonas FC
  const z = zonasFC();
  $('#nota-zonas').textContent = z.total
    ? `Reparto del tiempo en carrera — últimos 30 días (${fmtHoras(z.total)})`
  ...
  $('#zonas-lista').innerHTML = z.lista.map(x => `
    <div class="zona-fila">
      ...
      <span class="zona-rango">${x.rango}</span>
      <span class="zona-tiempo">${x.minutos ? fmtHoras(x.minutos) : '—'}</span>
    </div>`).join('');
}

function renderAvisoCarga() {
  const el = $('#aviso-carga');
  if (!el) return;
  const c = cargaRelativa();
  if (c.nivel === 'sin-datos') { el.hidden = true; return; }
  el.hidden = false;
  el.className = 'aviso aviso-' + c.nivel;
  el.innerHTML = `<strong>${escapar(c.texto)}</strong>${c.detalle ? `<span>${escapar(c.detalle)}</span>` : ''}`;
}
...
function renderPlanHoy() {
  ...
  if (!el) return;
  const hoy = hoyISO();
  const plan = parsearPlan(planDe(claveSemana(hoy)));
  const texto = plan[(new Date().getDay() + 6) % 7];
  if (!texto) { el.hidden = true; return; }
  const hecho = resumenDia(hoy);
  el.hidden = false;
  el.innerHTML = `<p class="eyebrow">Hoy toca</p>
    <p class="plan-hoy-txt">${escapar(texto)}</p>
    <p class="hint">${hecho ? 'Registrado: ' + escapar(hecho) : 'Todavía sin registrar'}</p>`;
}

function renderPlan(iso) {
  const ta = $('#plan-texto');
  if (!ta || !iso) return;
  ta.dataset.semana = iso;
  $('#plan-etiqueta').textContent = 'Semana del ' + fechaLarga(iso);
  const guardado = planDe(iso);
  ta.value = guardado;
  $('#plan-estado').textContent = guardado ? 'Guardado' : 'Sin plan para esta semana';

  const plan = parsearPlan(guardado);
  const cont = $('#plan-comparacion');
  if (!guardado.trim()) { cont.innerHTML = ''; return; }
  const lunes = desdeISO(iso);
  const hoy = hoyISO();
  cont.innerHTML = plan.map((texto, i) => {
    const d = new Date(lunes); d.setDate(lunes.getDate() + i);
    const dia = isoLocal(d);
    const hecho = resumenDia(dia);
    const eraDescanso = sinTildes(texto).includes('descanso');
    let estado = 'neutro', etiqueta = '—';
    if (dia > hoy) { estado = 'futuro'; etiqueta = 'Por venir'; }
    else if (hecho) { estado = 'ok'; etiqueta = 'Hecho'; }
    else if (eraDescanso) { estado = 'ok'; etiqueta = 'Descanso'; }
    else if (texto) { estado = 'no'; etiqueta = 'Sin registrar'; }
    return `<div class="plan-fila plan-${estado}">
      <span class="plan-dia">${DIAS_LARGOS[i].slice(0, 3)} ${fechaCorta(dia)}</span>
      <span class="plan-previsto">${texto ? escapar(texto) : '<em>sin nada previsto</em>'}</span>
      <span class="plan-hecho">${hecho ? escapar(hecho) : ''}</span>
      <span class="plan-estado">${etiqueta}</span>
    </div>`;
  }).join('');
}
...
function renderListaRunning() {
  ...
  cont.innerHTML = lista.slice(0, 20).map(e => {
    ...
    let zona = '';
    const nZona = numeroZona(e.fcMedia);
    if (nZona) {
      const c = ZONAS_FC[nZona - 1].color;
      zona = `<span class="badge" style="background:${c}28;color:${c}">Z${nZona}</span>`;
    }
    return `
      <div class="item">
        ...
        ${zona}
        <button class="btn-editar" data-editar="running" data-id="${e.id}" title="Editar">✎</button>
        <button class="btn-borrar" data-borrar="running" data-id="${e.id}" title="Eliminar">✕</button>
      </div>`;
  }).join('');
}

function renderListaBici() {
  const cont = $('#lista-bici');
  const lista = [...datos.bici].reverse();
  if (!lista.length) { cont.innerHTML = '<p class="vacio">Todavía no has registrado ninguna salida en bici.</p>'; return; }
  cont.innerHTML = lista.slice(0, 20).map(e => {
    const vel = e.duracion > 0 ? (e.distancia / (e.duracion / 60)) : 0;
    return `
      <div class="item">
        <span class="punto" style="background:#5c7cff"></span>
        <div class="cuerpo">
          <div class="fecha">${fechaLarga(e.fecha)} · ${escapar(e.tipo)}</div>
          <div class="titulo">${num(e.distancia, 2)} km · ${num(vel, 1)} km/h</div>
          <div class="detalle">${fmtTiempo(e.duracion)}${e.desnivel ? ` · ${num(e.desnivel, 0)} m D+` : ''}${e.fcMedia ? ` · FC ${e.fcMedia} ppm` : ''}${e.rpe != null ? ` · RPE ${e.rpe}` : ''}</div>
          ${e.notas ? `<div class="detalle">${escapar(e.notas)}</div>` : ''}
        </div>
        <button class="btn-editar" data-editar="bici" data-id="${e.id}" title="Editar">✎</button>
        <button class="btn-borrar" data-borrar="bici" data-id="${e.id}" title="Eliminar">✕</button>
      </div>`;
  }).join('');
}
...
function renderSemana() {
  const elegida = $('#form-semana').fecha.value;
  rellenarSemana(elegida ? claveSemana(elegida) : semanaActual().inicio, false);
  ...
  cont.innerHTML = lista.map(e => {
    ...
    return `
      <div class="item item-semana">
       <div class="fila-semana">
        <div class="cuerpo" data-abrir="${e.fecha}" style="cursor:pointer">
       ...
       <div class="md-caja" data-caja="${e.fecha}" hidden>
         <div class="md-acciones">
           <button type="button" class="btn-secundario" data-md="${e.fecha}">Descargar .md</button>
           <button type="button" class="btn-secundario" data-copiar="${e.fecha}">Copiar al portapapeles</button>
         </div>
         <pre data-pre="${e.fecha}"></pre>
       </div>
      </div>`;
  }).join('');
}
...
function renderHistorial() {
  const busca = sinTildes(($('#hist-busca')?.value || '').trim());
  ...
  const todo = [
    ...datos.running.map(e => ({
      ...
      busca: [e.tipo, e.notas, e.dolorZona, 'running', 'carrera'].join(' ')
    })),
    ...datos.bici.map(e => ({
      tipo: 'bici', fecha: e.fecha, id: e.id,
      titulo: `${num(e.distancia, 2)} km en bici · ${escapar(e.tipo)}`,
      extra: e.duracion > 0 ? `${num(e.distancia / (e.duracion / 60), 1)} km/h` : '',
      busca: [e.tipo, e.notas, 'bici'].join(' ')
    })),
    ...datos.fuerza.map(e => ({
      ...
      busca: [e.ejercicio, e.notas, 'fuerza'].join(' ')
    }))
  ].sort((a, b) => b.fecha.localeCompare(a.fecha))
    .filter(e => filtroHistorial === 'todos' || e.tipo === filtroHistorial)
    .filter(e => !desde || e.fecha >= desde)
    .filter(e => !hasta || e.fecha <= hasta)
    .filter(e => !busca || sinTildes(e.busca + ' ' + e.titulo).includes(busca));

  const cuenta = $('#hist-cuenta');
  const filtrando = busca || desde || hasta || filtroHistorial !== 'todos';
  if (cuenta) cuenta.textContent = filtrando
    ? `${todo.length} ${todo.length === 1 ? 'registro' : 'registros'} con los filtros puestos`
    : `${todo.length} ${todo.length === 1 ? 'registro' : 'registros'} en total`;

  const cont = $('#lista-historial');
  if (!todo.length) {
    cont.innerHTML = `<p class="vacio">${filtrando ? 'Ningún entreno encaja con lo que buscas.' : 'No hay entrenos que mostrar.'}</p>`;
    return;
  }
  cont.innerHTML = todo.map(e => `
    <div class="item">
      <span class="badge ${e.tipo}">${e.tipo === 'running' ? 'Running' : e.tipo === 'bici' ? 'Bici' : 'Fuerza'}</span>
      <div class="cuerpo">
        <div class="titulo">${e.titulo}</div>
        <div class="detalle">${fechaLarga(e.fecha)}</div>
      </div>
      <span class="extra">${e.extra}</span>
      <button class="btn-editar" data-editar="${e.tipo}" data-id="${e.id}" title="Editar">✎</button>
...
$('#form-running').addEventListener('submit', async e => {
  ...
  const editandoEste = editando && editando.col === 'running' ? editando.id : null;
  const entreno = {
    id: editandoEste || generarId(),
    fecha: d.fecha,
    distancia: dec(d.distancia),
    duracion: duracionDeTexto(d.duracion),
    tipo: d.tipo,
  ...
  if (!entreno.distancia || !entreno.duracion) {
    return avisar('Revisa la distancia y la duración. La duración se escribe como 46:15.', 'error');
  }
  cargando(true);
  try {
    await guardar('running', entreno);
    datos.running = datos.running.filter(x => x.id !== entreno.id);
    datos.running.push(entreno);
    datos.running.sort((a, b) => a.fecha.localeCompare(b.fecha));
    editando = null;
    cancelarEdicion('running');
    $('#nota-import').hidden = true;
    renderizar();
    irA('resumen');
    avisar(editandoEste ? 'Entreno actualizado' : 'Entreno guardado');
  } catch (err) { console.error(err); avisar('No se ha podido guardar el entreno.', 'error'); }
  finally { cargando(false); }
});

/* --- ritmo medio: distancia, duración y ritmo van siempre cuadrados.
   El ritmo no se guarda: es distancia y duración lo que define el entreno,
   así que guardar los tres invitaría a que se contradijeran. --- */
const fRun = $('#form-running');
let ritmoEscritoAMano = false;

function sincronizarRitmo(origen) {
  const km = dec(fRun.distancia.value);
  const min = duracionDeTexto(fRun.duracion.value);
  const rit = duracionDeTexto(fRun.ritmo.value);
  const hint = $('#hint-ritmo');

  if (origen === 'ritmo') {
    ritmoEscritoAMano = true;
    if (km > 0 && rit) {
      fRun.duracion.value = fmtTiempo(km * rit);
      hint.textContent = `Duración recalculada: ${fmtTiempo(km * rit)} para ${num(km, 2)} km.`;
    } else if (rit && !km) {
      hint.textContent = 'Pon la distancia y calculo la duración.';
    }
    return;
  }

  // se ha tocado distancia o duración
  if (km > 0 && min) {
    const calculado = fmtRitmo(ritmo(km, min));
    fRun.ritmo.value = calculado;
    ritmoEscritoAMano = false;
    hint.textContent = `${calculado} min/km con ${num(km, 2)} km en ${fmtTiempo(min)}.`;
  } else if (km > 0 && rit && ritmoEscritoAMano) {
    fRun.duracion.value = fmtTiempo(km * rit);
    hint.textContent = `Duración recalculada: ${fmtTiempo(km * rit)}.`;
  } else {
    fRun.ritmo.value = '';
    hint.textContent = 'Se calcula solo con la distancia y la duración. Si lo escribes tú, se recalcula la duración.';
  }
}

fRun.distancia.addEventListener('input', () => sincronizarRitmo('distancia'));
fRun.duracion.addEventListener('input', () => sincronizarRitmo('duracion'));
fRun.ritmo.addEventListener('input', () => sincronizarRitmo('ritmo'));

/* --- bicicleta --- */
$('#escala-rpe-bici').innerHTML = Array.from({ length: 11 }, (_, n) =>
  `<button type="button" data-rpe="${n}" class="${n === 5 ? 'on' : ''}">${n}</button>`).join('');
$('#escala-rpe-bici').addEventListener('click', e => {
  const b = e.target.closest('[data-rpe]');
  if (!b) return;
  $$('#escala-rpe-bici button').forEach(x => x.classList.remove('on'));
  b.classList.add('on');
  $('#form-bici [name=rpe]').value = b.dataset.rpe;
});

$('#form-bici').addEventListener('submit', async e => {
  e.preventDefault();
  const d = Object.fromEntries(new FormData(e.target).entries());
  const editandoEsta = editando && editando.col === 'bici' ? editando.id : null;
  const salida = {
    id: editandoEsta || generarId(),
    fecha: d.fecha,
    distancia: dec(d.distancia),
    duracion: duracionDeTexto(d.duracion),
    tipo: d.tipo,
    desnivel: dec(d.desnivel),
    fcMedia: d.fcMedia ? parseInt(d.fcMedia, 10) : null,
    rpe: parseInt(d.rpe, 10),
    notas: (d.notas || '').trim()
  };
  if (!salida.distancia || !salida.duracion) {
    return avisar('Revisa la distancia y la duración. La duración se escribe como 1:35:15.', 'error');
  }
  cargando(true);
  try {
    await guardar('bici', salida);
    datos.bici = datos.bici.filter(x => x.id !== salida.id);
    datos.bici.push(salida);
    datos.bici.sort((a, b) => a.fecha.localeCompare(b.fecha));
    editando = null;
    cancelarEdicion('bici');
    renderizar();
    irA('resumen');
    avisar(editandoEsta ? 'Salida actualizada' : 'Salida guardada');
  } catch (err) { console.error(err); avisar('No se ha podido guardar la salida.', 'error'); }
  finally { cargando(false); }
});
...
$('#btn-anadir-ejercicio').addEventListener('click', () => {
  ...
  const peso = dec(f.peso.value);
  if (!ejercicio || !series || !reps || peso === null) {
    avisar('Rellena ejercicio, series, repeticiones y peso.', 'error');
    return;
  }
...
$('#form-fuerza').addEventListener('submit', async e => {
  ...
  try {
    ...
    renderizar();
    avisar('Sesión de fuerza guardada');
  } catch (err) { console.error(err); avisar('No se ha podido guardar la sesión.', 'error'); }
  finally { cargando(false); }
});
...
$('#form-semana').addEventListener('submit', async e => {
  e.preventDefault();
  const d = Object.fromEntries(new FormData(e.target).entries());
  const semana = claveSemana(d.fecha);
  const doc = {
    id: 'sem-' + semana,
    fecha: semana,
    peso: dec(d.peso),
    kmTotales: dec(d.kmTotales),
  ...
  try {
    ...
    datos.semanas = datos.semanas.filter(x => x.id !== doc.id);
    datos.semanas.push(doc);
    datos.semanas.sort((a, b) => a.fecha.localeCompare(b.fecha));
    renderizar();
    avisar('Check-in guardado');
  } catch (err) { console.error(err); avisar('No se ha podido guardar el check-in.', 'error'); }
  finally { cargando(false); }
});

$('#btn-guardar-plan').addEventListener('click', async () => {
  const ta = $('#plan-texto');
  const iso = ta.dataset.semana;
  if (!iso) return avisar('Elige antes la semana y pulsa OK.', 'error');
  cargando(true);
  try {
    const planes = { ...(datos.perfil.planes || {}), [iso]: ta.value.trim() };
    if (!ta.value.trim()) delete planes[iso];
    await guardarPerfil({ planes });
    renderPlan(iso);
    renderPlanHoy();
    $('#plan-estado').textContent = 'Guardado ✓';
    avisar('Plan guardado');
  } catch (err) { console.error(err); avisar('No se ha podido guardar el plan.', 'error'); }
  finally { cargando(false); }
});

$('#btn-borrar-plan').addEventListener('click', async () => {
  const iso = $('#plan-texto').dataset.semana;
  if (!iso || !planDe(iso)) return;
  if (!confirm('¿Borrar el plan de esta semana?')) return;
  cargando(true);
  try {
    const planes = { ...(datos.perfil.planes || {}) };
    delete planes[iso];
    await guardarPerfil({ planes });
    renderPlan(iso);
    renderPlanHoy();
...
$('#btn-autorrellenar').addEventListener('click', () => {
  const v = $('#form-semana').fecha.value || hoyISO();
  rellenarSemana(claveSemana(v), true);
});

$('#form-semana [name=fecha]').addEventListener('change', e => {
  if (e.target.value) rellenarSemana(claveSemana(e.target.value), true);
});
...
$('#form-perfil').addEventListener('submit', async e => {
  ...
  try {
    await guardarPerfil({
      ...
      objetivoKm: dec(d.objetivoKm) ?? 0,
      objetivoSesiones: d.objetivoSesiones ? parseInt(d.objetivoSesiones, 10) : 0
    });
    pintarUsuario();
    renderizar();
    avisar('Perfil guardado');
  } catch (err) { console.error(err); avisar('No se ha podido guardar el perfil.', 'error'); }
  finally { cargando(false); }
});

/* --- filtros historial --- */
let relojBusca = null;
$('#hist-busca').addEventListener('input', () => {
  clearTimeout(relojBusca);
  relojBusca = setTimeout(renderHistorial, 180);
});
$('#hist-desde').addEventListener('change', renderHistorial);
$('#hist-hasta').addEventListener('change', renderHistorial);
$('#hist-limpiar').addEventListener('click', () => {
  $('#hist-busca').value = '';
  $('#hist-desde').value = '';
  $('#hist-hasta').value = '';
  $$('#filtros-historial .pill').forEach(x => x.classList.remove('active'));
  $('#filtros-historial .pill[data-filtro=todos]').classList.add('active');
...
/* --- edición de un registro ya guardado --- */
let editando = null;

function textoBoton(col) { return col === 'running' ? 'Guardar entreno' : 'Guardar salida'; }

function actualizarModoEdicion(col) {
  const f = $('#form-' + col);
  if (!f) return;
  const activo = !!editando && editando.col === col;
  f.querySelector('button[type=submit]').textContent = activo ? 'Guardar cambios' : textoBoton(col);
  $('#edit-aviso-' + col).hidden = !activo;
}

function cancelarEdicion(col) {
  editando = null;
  const f = $('#form-' + col);
  f.reset();
  f.fecha.value = hoyISO();
  const porDefecto = col === 'running' ? 6 : 5;
  f.rpe.value = porDefecto;
  const escala = col === 'running' ? '#escala-rpe' : '#escala-rpe-bici';
  $$(escala + ' button').forEach(b => b.classList.toggle('on', +b.dataset.rpe === porDefecto));
  if (col === 'running') {
    ritmoEscritoAMano = false;
    $('#hint-ritmo').textContent = 'Se calcula solo con la distancia y la duración. Si lo escribes tú, se recalcula la duración.';
  }
  actualizarModoEdicion(col);
}

function editarRegistro(col, id) {
  if (col === 'fuerza') return editarFuerza(id);
  const e = datos[col].find(x => x.id === id);
  if (!e) return;
  editando = { col, id };
  const f = $('#form-' + col);
  f.fecha.value = e.fecha;
  f.distancia.value = e.distancia ?? '';
  f.duracion.value = e.duracion ? fmtTiempo(e.duracion) : '';
  if (col === 'running') f.ritmo.value = e.distancia && e.duracion ? fmtRitmo(ritmo(e.distancia, e.duracion)) : '';
  f.tipo.value = e.tipo || '';
  f.fcMedia.value = e.fcMedia ?? '';
  f.rpe.value = e.rpe ?? '';
  f.notas.value = e.notas || '';
  if (col === 'running') {
    f.fcMaxima.value = e.fcMaxima ?? '';
    f.rodilla.value = e.rodilla ?? 0;
    f.dolorValor.value = e.dolorValor ?? 0;
    f.dolorZona.value = e.dolorZona || '';
    $('#campos-avanzados').hidden = false;
    $('#btn-avanzado').textContent = '− Ocultar FC, dolor y notas';
  } else {
    f.desnivel.value = e.desnivel ?? '';
  }
  const escala = col === 'running' ? '#escala-rpe' : '#escala-rpe-bici';
  $$(escala + ' button').forEach(b => b.classList.toggle('on', +b.dataset.rpe === e.rpe));
  actualizarModoEdicion(col);
  irA(col);
}

function editarFuerza(id) {
  const x = datos.fuerza.find(e => e.id === id);
  if (!x) return;
  abrirModal('Editar ejercicio', [
    { html: `<label class="full">Ejercicio<input name="ejercicio" value="${escapar(x.ejercicio)}" required></label>` },
    { html: `<label>Fecha<input type="date" name="fecha" value="${x.fecha}" required></label>` },
    { html: `<label>Series<input type="number" min="1" name="series" value="${x.series}" required></label>` },
    { html: `<label>Repeticiones<input type="number" min="1" name="reps" value="${x.reps}" required></label>` },
    { html: `<label>Peso (kg)<input type="number" step="0.01" min="0" name="peso" value="${x.peso}" required></label>` },
    { html: `<label class="full">Notas<input name="notas" value="${escapar(x.notas || '')}"></label>` }
  ], async d => {
    const doc = {
      id, fecha: d.fecha, ejercicio: d.ejercicio.trim(),
      series: parseInt(d.series, 10), reps: parseInt(d.reps, 10),
      peso: dec(d.peso), notas: (d.notas || '').trim()
    };
    cargando(true);
    try {
      await guardar('fuerza', doc);
      datos.fuerza = datos.fuerza.filter(e => e.id !== id);
      datos.fuerza.push(doc);
      datos.fuerza.sort((a, b) => a.fecha.localeCompare(b.fecha));
      renderizar();
      avisar('Ejercicio actualizado');
  } catch (err) { console.error(err); avisar('No se ha podido guardar el cambio.', 'error'); }
    finally { cargando(false); }
  });
}
...
document.addEventListener('click', async e => {
  ...
  if (btnEditar) { editarRegistro(btnEditar.dataset.editar, btnEditar.dataset.id); return; }
  const btnCancelar = e.target.closest('[data-cancelar-edicion]');
  if (btnCancelar) { cancelarEdicion(btnCancelar.dataset.cancelarEdicion); return; }
  ...
  const btnDescanso = e.target.closest('[data-descanso]');
  if (btnDescanso) {
    const dia = btnDescanso.dataset.descanso;
    const lista = [...(datos.perfil.descansos || [])];
    const i = lista.indexOf(dia);
    if (i >= 0) lista.splice(i, 1); else lista.push(dia);
    lista.sort();
    btnDescanso.classList.toggle('on', i < 0);
    await guardarPerfil({ descansos: lista });
    renderResumen();
    return;
  }
  const abrir = e.target.closest('[data-abrir]');
  if (abrir) {
    const iso = abrir.dataset.abrir;
    const caja = document.querySelector(`[data-caja="${iso}"]`);
    if (caja) {
      if (caja.hidden) document.querySelector(`[data-pre="${iso}"]`).textContent = markdownSemana(iso);
      caja.hidden = !caja.hidden;
    }
    return;
  }
  const md = e.target.closest('[data-md]');
  if (md) { descargarMD(md.dataset.md); return; }
  const copiar = e.target.closest('[data-copiar]');
  if (copiar) {
    try {
      await navigator.clipboard.writeText(markdownSemana(copiar.dataset.copiar));
      const t = copiar.textContent;
      copiar.textContent = 'Copiado ✓';
      setTimeout(() => { copiar.textContent = t; }, 1600);
    } catch (_) { avisar('El navegador no ha permitido copiar. Usa «Descargar .md».', 'error'); }
    return;
  }
...
$('#input-archivo').addEventListener('change', async e => {
  ...
  try {
    ...
    f.duracion.value = a.minutos ? fmtTiempo(a.minutos) : '';
    f.ritmo.value = a.km && a.minutos ? fmtRitmo(ritmo(a.km, a.minutos)) : '';
    if (a.fcMedia) {
      $('#campos-avanzados').hidden = false;
      $('#btn-avanzado').textContent = '− Ocultar FC, dolor y notas';
...
$('#btn-pr-manual').addEventListener('click', () => {
  abrirModal('Récord a mano', [
    ...
    if (isNaN(tiempo)) return avisar('Escribe el tiempo como mm:ss o h:mm:ss.', 'error');
...
$('#btn-nueva-meta').addEventListener('click', () => {
  abrirModal('Nueva meta', [
    ...
    const objetivo = d.tipo === 'tiempo' ? minutosDeTexto(d.objetivo) : dec(d.objetivo);
    if (objetivo === null || isNaN(objetivo)) return avisar('El objetivo no es válido.', 'error');
    const metas = [...(datos.perfil.metas || []), {
...
$('#btn-borrar').addEventListener('click', async () => {
  ...
  cargando(true);
  try {
    for (const nombre of ['running', 'bici', 'fuerza', 'semanas']) {
    ...
    datos.running = []; datos.bici = []; datos.fuerza = []; datos.semanas = [];
    renderizar();
  } finally { cargando(false); }
});
...
$('#form-bici [name=fecha]').value = hoyISO();
