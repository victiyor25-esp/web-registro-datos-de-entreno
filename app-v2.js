...
$('#form-bici').addEventListener('submit', async e => {
  ...
  if (!salida.distancia || !salida.duracion) return alert('Rellena distancia y duración.');
  cargando(true);
  try {
...
$('#btn-guardar-plan').addEventListener('click', async () => {
  ...
  if (!iso) return alert('Elige antes la semana y pulsa OK.');
  cargando(true);
  try {
  ...
  } catch (err) { console.error(err); alert('No se ha podido guardar el plan.'); }
  finally { cargando(false); }
});
...
function editarFuerza(id) {
  ...
  abrirModal('Editar ejercicio', [
    ...
    try {
      ...
      datos.fuerza.sort((a, b) => a.fecha.localeCompare(b.fecha));
      renderizar();
    } catch (err) { console.error(err); alert('No se ha podido guardar el cambio.'); }
    finally { cargando(false); }
  });
}
...
document.addEventListener('click', async e => {
  ...
  if (copiar) {
    ...
    } catch (_) { alert('El navegador no ha permitido copiar. Usa «Descargar .md».'); }
    return;
  }
...
$('#btn-nueva-meta').addEventListener('click', () => {
  abrirModal('Nueva meta', [
    ...
    if (objetivo === null || isNaN(objetivo)) return alert('El objetivo no es válido.');
    const metas = [...(datos.perfil.metas || []), {
