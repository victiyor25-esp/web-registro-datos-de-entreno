...
function claveSemana(fecha) { return lunesDe(fecha).toISOString().slice(0, 10); }
...
function rachaSemanas() {
  ...
  if ((mapa.get(cursor.toISOString().slice(0, 10)) || 0) >= objetivo) cuenta++;
  cursor.setDate(cursor.getDate() - 7);
  while ((mapa.get(cursor.toISOString().slice(0, 10)) || 0) >= objetivo) {
    cuenta++;
    cursor.setDate(cursor.getDate() - 7);
  }
  return cuenta;
...
function semanaActual() {
  const inicio = lunesDe(new Date()).toISOString().slice(0, 10);
...
function zonasFC() {
  ...
  const conFC = datos.running.filter(e => e.fcMedia && new Date(e.fecha) >= desde);
...
function renderResumen() {
  ...
  $('#semana-dias').innerHTML = nombres.map((n, i) => {
    ...
    return `<div class="dia ${sem.fechas.has(iso) ? 'on' : ''} ${iso === hoy ? 'hoy' : ''}">
  ...
  const diasSinDolor = ultRodilla
    ? Math.round((Date.now() - new Date(ultRodilla.fecha)) / 86400000)
    : null;
